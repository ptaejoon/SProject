    ...
    try:
        ...
        if len(qs_img) == 0 :
            ...
            for img in qs_img:
                ...
                s3.Bucket(BUCKET).upload_file(img_name,img_name)
        else :
            for img in qs_img:
                img_src = img.get('src')
                img_alt = img.get('alt')
        ...
        s3.Bucket(BUCKET).upload_file(pd_img_name,pd_img_name)
        ...
        f.close()
        s3.Bucket(BUCKET).upload_file(raw_name,raw_name)
        #텍스트 정보 가져오기
        ...
        for ti in textInfo:
            f.write(ti.get_text()+'\n')
        f.close()
        s3.Bucket(BUCKET).upload_file(textname,textname)
        return 0
...
def onePageExtracting(url):
    ...
    for arg in lists:
        ...
        sleeptime = random.randrange(10,15)
...
s3 = boto3.resource('s3')
BUCKET = 'beginveganscrapdata'
s3.Bucket(BUCKET).upload_file('collectedFailed.txt','collectedFailed.txt')
