...
def getName(url):
    url = url.replace(', 신세계적 쇼핑포털 SSG.COMtextInfo.txt','')
...
category_sql = """INSERT INTO product_categories ( name, category ) VALUES ( %s, %s ) ON DUPLICATE KEY UPDATE name = VALUES(name)"""
bucket = s3.Bucket(name = 'beginveganscrapdata').objects.filter(Prefix='text/')
for obj in bucket:

    if obj.key == 'text/':
        continue
    name = getName(obj.key)
    f = obj.get()['Body'].read().decode('utf-8')
    cat = False;
    category = '-'
    for sentence in f.split('\n'):
        if sentence == '식품의 유형' or sentence == '식품의유형':
            cat = True
            continue;
        if cat is True:
            category = sentence
            cat = False
            break
    cursor.execute(category_sql, (name, sentence))
    connect.commit()
