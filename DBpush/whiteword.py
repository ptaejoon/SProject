...
#get_whiteword : 유저 딕셔너리에서 식품 리스트 가져옴
...
def checkWhite(word_list,test_list):
	for tl in test_list:
		for wl in word_list:
			if tl == wl:
				return True
...
def start(texts,jamo,whiteword,komoran):
	jamo_dict = jamo
	komo = komoran
	fd_list = []
	fir_list = []
	...
	for oneLine in f:
		...
		for word_list in splited_word:
			if word_list in cautionwords:
				cau_list.append(oneLine)
				break
			if word_list in stopwords:
				stop_list.append(oneLine)
				break
		#스톱 워드를 포함한 sentence는 따로 저장
		else:
			fir_list.append(oneLine)
			splited_data = spliting(oneLine,replacewords)
			if checkWhite(whiteword,splited_data) is True:
				#print(splited_data)
				sec_list.append(splited_data)
				for sl_data in splited_data:
					food = find_most_similar(sl_data,jamo_dict,0.9)
					if food is not ' ' and food not in fd_list:
						fd_list.append(food)
	...
	return fd_list,fir_list,sec_list,cau_list,stop_list
